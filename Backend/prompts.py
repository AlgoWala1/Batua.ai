...
6. When user just mentions a time period in almost all of the cases the end date will be today's date.
INTENTS:
...
INTENT 3: If the query ask for movement of certain benchmark indexes(or sectors of that index) over a period of time:
  ...
  - "In a five year period, how has the IT sector performed?"
         In this query also end date will be today's date that is {last_business_day} and start date will be 5 years before {last_business_day}
...
INTENT 5: If the query ask for technical analysis of stock
  Example queries:
...
INTENT 6: If the query asks for debt vs equity allocation based on age/risk profile:
  Example queries:      
...
RESPONSE_PROMPT = """
      ...
      Example:
      Query: "Show me the technial analysis of Bank of Maharashtra over 60 days period"
      JSON:
      {
        "Analysis": {
            "Averages": [
                {
                    "Moving average(MA) 20 days": 54.00449962615967
                },
                {
                    "Exponential Moving average(EMA) 20 days": 55.006191081577754
                },
                {
                    "Moving average(MA) 50 days": 55.05039978027344
                },
                {
                    "Exponential Moving average(EMA) 50 days": 54.64284738044755
                }
            ],
            "Oscillators": [
                "RSI: 51.19048899005789",
                "Stochastic %K (14): 88.99832566739974",
                "Commodity channel index(CCI 20): 145.88298160893083"
            ],
            "Current stock price": 57.2599983215332
        }
      }
...
Disclaimer: This analysis is for informational purposes only and not a recommendation for trading.
      TECHNICAL ANALYSIS:
        - RSI: Interpret RSI: >70 → overbought (possible pullback), <30 → oversold (possible rebound), 30-70 → neutral.
        - Stochastic %K: Interpret %K: >80 → overbought (possible pullback), <20 → oversold (possible rebound), crossing %D → potential short-term buy/sell signal.
